  ...
  <div>
      ...
      <b-col v-for="(it, index) in items" :key="index" md="6" sm="12" style="margin-top: 10px;">
        <b-card bg-variant="light" text-variant="dark" :title="it.name">
          ...
          <b-button-group size="sm" class="float-right">
            <b-button variant="link" :to="`/projects/${it.id}`" title="进入"><span class="oi oi-list"></span>
            </b-button>
            <b-button variant="link" title="修改" @click="handleShowEditModal(it)"><span class="oi oi-pencil"></span>
            </b-button>
            <b-button variant="link" title="删除" @click="handleDelete(it.id)"><span class="oi oi-delete"></span>
            </b-button>
          </b-button-group>
        </b-card>
      </b-col>
    ...
    <b-modal
      ...
      <component
        ...
        :item.sync="data_item"
        @project-add-save="handleAddSave"
        @project-update-save="handleUpdateSave"
      ></component>
    </b-modal>
  </div>
</template>
